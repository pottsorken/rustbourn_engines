...
use crate::{
    common::{
        ...
        PlayerGrid,HookRange, BLOCK_CONFIG, HOOK_CONFIG, OBSTACLE_CONFIG, PLAYER_CONFIG,
    },
...
pub fn hook_controls(
    ...
    for (mut sprite, mut transform, hook, mut charge) in query.p0().iter_mut() {
        if keyboard_input.pressed(KeyCode::Space) && charge.target_length == 0.0 {
            ...
            let estimated_range = (charge.time_held / 2.0 * hook.hook_speed).min(hook.hook_max_range);
